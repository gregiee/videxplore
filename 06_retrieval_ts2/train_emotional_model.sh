...
if [ ! -d "$OUTPUT_ROOT/emotional" ]; then
  mkdir -p "$OUTPUT_ROOT/emotional"
fi

python -m torch.distributed.launch --nproc_per_node=1 \
main_task_retrieval.py \
--train_csv "${ANNOTATION_ROOT}/train.csv" \
--val_csv   "${ANNOTATION_ROOT}/emotional_test.csv" \
--data_path "${ANNOTATION_ROOT}/emotional_captions.json" \
--features_path "${CLIP_ROOT}/videos" \
--output_dir    "${OUTPUT_ROOT}/emotional" \
\
--do_train \
--eval_in_train \
--num_thread_reader=4 \
