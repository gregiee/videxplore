...
from modules.until_module import PreTrainedModel, AllGather, CrossEn, BTloss, ClassifyCrossEn, TextPromptEncoder, \
    VideoPromptEncoder, make_patch_shift, make_attn_visual, make_token_shuffle
from modules.module_cross import CrossModel, CrossConfig, Transformer as TransformerClip
from modules.differential_topk import VisualTokenSelection, TextTokenSelection, VisualTokenRandomSelection, \
    STVisualTokenSelection
...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None):
        ...
        if self.training:
            ...
            sim_matrix_semantic, sim_matrix_global = self.get_similarity_logits(sequence_output, visual_output,
    ...
    def get_visual_output(self, video, video_mask, shaped=False, video_frame=-1):
        ...
        visual_hidden = visual_hidden.view(bs_pair, -1, visual_hidden.size(
            -1))  # shape here should be (bs, max_frames*sample_len, hid_dim)
    ...
    def get_sequence_output_emb(self, input_ids, token_type_ids, attention_mask, shaped=False):
        if shaped is False:
            input_ids = input_ids.view(-1, input_ids.shape[-1])
            token_type_ids = token_type_ids.view(-1, token_type_ids.shape[-1])
            attention_mask = attention_mask.view(-1, attention_mask.shape[-1])
    ...
    def get_visual_output_emb(self, video, video_mask, shaped=False, video_frame=-1):
        if shaped is False:
            video_mask = video_mask.view(-1, video_mask.shape[-1])
    ...
    def _loose_similarity(self, sequence_output, visual_output, attention_mask, video_mask, sim_header="meanP"):
        ...
        # get frame sim
        sim_matrix_semantic = self.get_frame_selectedcls_similarity(sequence_output, visual_output, attention_mask,
                                                                    video_mask)
    ...
    def get_similarity_logits(self, sequence_output, visual_output, attention_mask, video_mask, shaped=False,
        ...
        if loose_type:
            ...
            sim_matrix_global, sim_matrix_semantic = self._loose_similarity(sequence_output, visual_output,
                                                                            attention_mask, video_mask,
    ...
    def get_final_similarity(self, sequence_output, visual_output, attention_mask, video_mask, shaped=False,
                             loose_type=False):
        sim_matrix_semantic, sim_matrix_global = self.get_similarity_logits(sequence_output, visual_output,
                                                                            attention_mask, video_mask, shaped=shaped,
    ...
    def get_frame_similarity(self, sequence_output, visual_output, attention_mask, video_mask):
        ...
        ##################################################################
        similarity_matrix = torch.einsum('mjk,nlk->mjn', visual_output,
                                         sequence_output)  # shape here should be(bs_v, max_frames, bs_s)
        ...
        similarity_matrix_weight = torch.softmax(4 * similarity_matrix_weight,
                                                 dim=1)  # normalization between frames for each frame
    ...
    def get_frame_selectedcls_similarity(self, sequence_output, visual_output, attention_mask, video_mask):
        ...
        visual_output = visual_output / visual_output.norm(dim=-1,
                                                           keepdim=True)  # shape here is (bs, max_frames, hid_dim)

        video_mask_un = video_mask.to(dtype=torch.bool).unsqueeze(-1)
        ##################################################################
        ## should change to visual matmul sequence due to match in test ##
        ##################################################################
        similarity_matrix = torch.einsum('mjk,nlk->mjln', visual_output,
                                         sequence_output)  # shape here should be(bs_v, max_frames, 2, bs_s)
